...
// Active/selected item — matches the left-nav active state: white interior + faint green hairline.
const navSelected = {
  background: "#FFFFFF",
  border: "1px solid rgba(0,122,114,0.35)",
...
const providers = [
  ...
  { id: "chen", name: "Dr. Sarah Chen", specialty: "Family Medicine", initials: "SC", location: "Main Campus", next: "Tomorrow, 9:00 AM", color: "#00A499" },
  ...
  { id: "park", name: "Dr. Emily Park", specialty: "Cardiology", initials: "EP", location: "Main Campus", next: "Fri Apr 12, 11:00 AM", color: "#004D4D" },
];
...
function CompletedSummary({ label, icon: IconComp }) {
  return (
    <div className="flex items-center gap-2.5 text-sm" style={{ color: colors.textDark }}>
      {IconComp && <IconComp size={15} style={{ color: "#00A9A0", flexShrink: 0 }} />}
        ...
        {careTypes.map((ct) => {
          ...
          return (
            <OptionCard key={ct.id} selected={isSelected} onClick={() => onSelect(ct)}
              selectedStyle={{ background: "#FFFFFF", border: "1px solid rgba(0,122,114,0.35)", boxShadow: glass.boxShadow }}>
              <div className="flex items-center gap-3">
                <span className="flex items-center justify-center rounded-xl flex-shrink-0"
                  style={{
                    width: 38, height: 38,
                    background: isSelected ? "#00A9A0" : "#FFFFFF",
                    color: isSelected ? "#FFFFFF" : "#00A9A0"
                  }}>
                  <IconComp size={18} />
                </span>
                <div className="min-w-0">
                  <p className="font-normal text-sm leading-tight uppercase tracking-wide" style={{ color: colors.primary }}>{ct.label}</p>
                  <p className="text-xs mt-0.5 leading-tight" style={{ color: colors.textDark }}>{ct.desc}</p>
                </div>
                {isSelected && <Check size={16} className="ml-auto flex-shrink-0" style={{ color: "#00A9A0" }} />}
              </div>
            </OptionCard>
          );
        })}
              ...
              <div className="flex items-center gap-3.5">
                ...
                {isFirstAvail ? (
                  <span className="flex items-center justify-center rounded-full flex-shrink-0"
                    style={{ width: 42, height: 42, background: "#FFFFFF", color: "#00A9A0" }}>
                    <UserIcon size={20} />
                  </span>
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-normal text-sm leading-tight uppercase tracking-wide" style={{ color: colors.primary }}>{prov.name}</p>
                  <p className="text-xs mt-0.5" style={{ color: colors.textDark }}>{prov.specialty}</p>
                  <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                    ...
                    <span className="flex items-center gap-1 text-xs font-medium" style={{ color: "#00A9A0" }}>
                      <Clock size={11} /> Next: {prov.next}
                    </span>
                  </div>
                </div>
                {isSelected && <Check size={16} className="flex-shrink-0" style={{ color: "#00A9A0" }} />}
              </div>
            </OptionCard>
          );
        })}
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {formats.map((fmt) => {
          ...
          return (
            <OptionCard key={fmt.id} selected={isSelected} onClick={() => onSelect(fmt)} selectedStyle={navSelected}>
              <div className="flex flex-col gap-3 p-1">
                ...
                <div>
                  <p className="font-normal text-sm uppercase tracking-wide" style={{ color: colors.primary }}>{fmt.label}</p>
                  <p className="text-xs mt-1 leading-relaxed" style={{ color: colors.textDark }}>{fmt.desc}</p>
                </div>
                {isSelected && (
                  <span className="inline-flex items-center gap-1 text-xs font-medium rounded-full px-2 py-0.5 self-start"
                    style={{ background: "rgba(0,169,160,0.1)", color: "#00A9A0", border: `1px solid rgba(0,169,160,0.25)` }}>
    ...
    <div>
      ...
      <div className="flex flex-col gap-2.5">
        {locations.map((loc) => {
          ...
          return (
            <OptionCard key={loc.id} selected={isSelected} onClick={() => onSelect(loc)} selectedStyle={navSelected}>
              <div className="flex items-start gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-normal text-sm leading-tight uppercase tracking-wide" style={{ color: colors.primary }}>{loc.name}</p>
                  <p className="text-xs mt-1" style={{ color: colors.textDark }}>{loc.address}</p>
                  <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                    <span className="text-xs font-medium" style={{ color: "#00A9A0" }}>{loc.distance} away</span>
        ...
        {availableDates.map((d) => {
          ...
          return (
            <button key={key}
              ...
              style={isSelected ? navSelected : glass}
              aria-pressed={isSelected}
              aria-label={`${dayNames[d.getDay()]} ${monthNames[d.getMonth()]} ${d.getDate()}`}>
              <span className="text-xs font-medium" style={{ color: isSelected ? "#00A9A0" : colors.textMuted }}>
                {dayNames[d.getDay()]}
              </span>
              <span className="text-lg font-bold leading-tight mt-0.5" style={{ color: isSelected ? "#00A9A0" : colors.textDark }}>
                {d.getDate()}
              </span>
              <span className="text-xs" style={{ color: isSelected ? "#00A9A0" : colors.textMuted }}>
              ...
              {morningSlots.map((slot) => {
                ...
                return (
                  <button key={slot} disabled={isUnavail} onClick={() => onTimeSelect(slot)}
                    ...
                    <span style={{ color: isSelected ? "#00A9A0" : isUnavail ? colors.textMuted : colors.textDark }}>{slot}</span>
                  </button>
                );
              })}
            </div>
  ...
  return (
    <div>
        ...
        {rows.map((row, i) => {
          ...
          return (
            <div key={row.label}
              ...
              style={{
                background: i % 2 === 0 ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.2)",
              ...
              <span className="flex items-center justify-center rounded-lg flex-shrink-0"
                style={{ width: 30, height: 30, background: "#FFFFFF", color: "#00A9A0" }}>
                <IconComp size={15} />
              </span>
      ...
      <div className="flex items-center gap-2.5 rounded-xl px-4 py-3 mb-5"
        style={{ background: "rgba(0,122,114,0.07)", border: "1px solid rgba(0,122,114,0.2)" }}>
        <Clock size={15} style={{ color: "#00A9A0", flexShrink: 0 }} />
      ...
      <button onClick={onConfirm}
        className="w-full rounded-2xl py-4 font-semibold text-sm transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
        style={{
      ...
      <div className="px-5 pt-5 pb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.5)" }}>
        ...
        <div className="mt-3 rounded-full overflow-hidden" style={{ height: 4, background: "rgba(255,255,255,0.4)" }}>
          <div className="h-full rounded-full transition-all duration-500"
            style={{
              width: `${(filledCount / summaryRows.length) * 100}%`,
              background: `linear-gradient(90deg, ${colors.primary}, #00A499)`
            }} />
        </div>
      </div>
          ...
          return (
            <div key={row.label} className="flex items-center gap-3">
              <span className="flex items-center justify-center rounded-lg flex-shrink-0"
                style={{
                  width: 28, height: 28,
                  background: row.filled ? "#00A9A0" : "#FFFFFF",
                  color: row.filled ? "#FFFFFF" : "#00A9A0"
                }}>
                <IconComp size={13} />
              </span>
              <div className="min-w-0">
  ...
  return (
    <div className="flex flex-col items-center text-center py-12 px-6">
      <div className="flex items-center justify-center rounded-full mb-5"
        style={{ width: 72, height: 72, background: "#FFFFFF", boxShadow: glass.boxShadow }}>
        <CheckCircle size={36} style={{ color: "#00A9A0" }} />
      </div>
    ...
    <aside className="rounded-2xl px-5 py-5" style={{ ...glass }}>
      ...
      <div className="flex flex-col gap-2">
        <a href="#messages"
          ...
          style={{ color: colors.primary, background: "rgba(0,122,114,0.08)", border: "1px solid rgba(0,122,114,0.2)" }}
          onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(0,122,114,0.14)")}
          onMouseLeave={(e) => (e.currentTarget.style.background = "rgba(0,122,114,0.08)")}>
...
function BookingStepsCarousel3D({
  ...
  // Larger perspective ⇒ the active card is magnified far less by translateZ
  // (stageScale ≈ 1.04 instead of 1.22), so its rasterized text stays crisp.
  const PERSP = isMobile ? 6500 : 10000;
  const stageScale = PERSP / (PERSP - RADIUS);
  // Card authored slightly larger so its projected footprint is unchanged.
  const CARD_W = isMobile ? (windowWidth - 40) / stageScale : 505;
                ...
                {active && (
                  <div className="flex items-start gap-3"
                    ...
                    <span className="flex-shrink-0 flex items-center justify-center rounded-full w-7 h-7 text-xs font-bold"
                      style={{ background: "#00A9A0", color: "#fff" }}>
                      {activeIndex + 1}
                    </span>
